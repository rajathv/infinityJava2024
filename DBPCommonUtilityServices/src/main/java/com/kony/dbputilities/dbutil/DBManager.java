...
public class DBManager {
    ...
    private static synchronized void createDataSource(FabricRequestManager requestManager) {
        if (DATASOURCE == null || DATASOURCE.isClosed()) {

            Properties properties = new Properties();
            String jdbcUrl = URLFinder.getPathUrl(URLConstants.DB_JDBCURL, requestManager);
            String userName = URLFinder.getPathUrl(URLConstants.DB_USERNAME, requestManager);
            String password = URLFinder.getPathUrl(URLConstants.DB_PASSWORD, requestManager);
    ...
    public static Connection getConnection(FabricRequestManager requestManager) {

        try {
            if (DATASOURCE == null || DATASOURCE.isClosed()) {
                createDataSource(requestManager);
            }
            return DATASOURCE.getConnection();
        } catch (Exception e) {
            LOG.error(e.getMessage());
