...
public class AdminUtil {
    ...
    public static Result invokeAPI(Map<String, String> inputParams, String relativeURL) {
        ...
        headerMap.put("X-Kony-AC-API-Access-By", "OLB");
        headerMap.put(HttpHeaders.CONTENT_TYPE, ContentType.APPLICATION_JSON.getMimeType());
        LOG.debug("request payload : "+inputParams);
        LOG.debug("headers payload : "+headerMap);
        try {
        	Result result = ServiceCallHelper.invokeServiceAndGetResult(HelperMethods.convertToObjectMap(inputParams), headerMap,
                    relativeURL, backendToken);
        	LOG.debug("response payload : " + ResultToJSON.debugConvert(result));
        	return result;
        } catch (Exception e) {
            LOG.error("Exception while calling relativeURL" + relativeURL, e);
            return ErrorCodeEnum.ERR_10100.setErrorCode(new Result());
        }
    }
    ...
    public static String getForceLoginAccessToken(DataControllerRequest dcRequest) throws HttpCallException {
        ...
        LOG.error("getForceLoginAccessToken.headers "+headers);
        ...
        LOG.error("getForceLoginAccessToken.response "+response);
    ...
    public static String getAdminToken(DataControllerRequest requestInstance) {
        ...
        LOG.error("getAdminToken "+ACCESS_TOKEN);
        return ACCESS_TOKEN;
    }
    ...
    /*
     * This method return the Authentication Token From Quantum
     */
	public static String getQuantumAuthToken(DataControllerRequest dcRequest) {
		String userId = EnvironmentConfigurationsHandler.getServerProperty("DBX_FABRIC_USERNAME");
		String password = EnvironmentConfigurationsHandler.getServerProperty("DBX_FABRIC_PASSWORD");
		...
		try {
			JsonObject response = ServiceCallHelper.invokeServiceAndGetJson(inputParams, dcRequest.getHeaderMap(),
					URLConstants.QUANTUM_LOGIN);
			if (response != null && !response.isJsonNull() && !response.has("errmsg") && response.has("claimsToken")) {
				claimsToken = response.get("claimsToken").getAsString();
				return claimsToken;
			} else {
				LOG.error("Failed to fetch Quantum Auth Token, Invalid UserId or Password");
				return null;
			}
		} catch (Exception e) {
			LOG.error("Exception occured in calling Quantum login");
			return null;
		}
	}
}
