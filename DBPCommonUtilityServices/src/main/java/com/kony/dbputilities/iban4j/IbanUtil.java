...
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.BBAN_LENGTH;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.BBAN_ONLY_DIGITS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.BBAN_ONLY_DIGITS_OR_LETTERS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.BBAN_ONLY_UPPER_CASE_LETTERS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.CHECK_DIGIT_ONLY_DIGITS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.CHECK_DIGIT_TWO_DIGITS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.COUNTRY_CODE_EXISTS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.COUNTRY_CODE_TWO_LETTERS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.COUNTRY_CODE_UPPER_CASE_LETTERS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.IBAN_FORMATTING;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.IBAN_NOT_EMPTY;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.IBAN_NOT_NULL;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.IBAN_VALID_CHARACTERS;
import static com.kony.dbputilities.iban4j.IbanFormatException.IbanFormatViolation.UNKNOWN;
...
public final class IbanUtil {
     ...
     *            to be validated.
     * @throws IbanFormatException
     *             if iban is invalid. UnsupportedCountryException if iban's country is not supported.
        ...
        switch (format) {
            case Default:
                ...
                if (!toFormattedString(ibanWithoutSpaces).equals(iban)) {
                    throw new IbanFormatException(IBAN_FORMATTING, String.format(
                            "Iban must be formatted using 4 characters and space combination. " + "Instead of [%s]",
                            iban));
                }
                break;
            default:
    ...
    private static void validateCheckDigit(final String iban) {
        if (calculateMod(iban) != 1) {
            ...
            throw new InvalidCheckDigitException(checkDigit, expectedCheckDigit,
                    String.format("[%s] has invalid check digit: %s, " + "expected check digit is: %s", iban,
    ...
    private static void validateCheckDigitPresence(final String iban) {
        ...
        if (iban.length() < COUNTRY_CODE_LENGTH + CHECK_DIGIT_LENGTH) {
            throw new IbanFormatException(CHECK_DIGIT_TWO_DIGITS, iban.substring(COUNTRY_CODE_LENGTH),
    ...
    private static void validateBbanLength(final String iban, final BbanStructure structure) {
        ...
        if (expectedBbanLength != bbanLength) {
            throw new IbanFormatException(BBAN_LENGTH, bbanLength, expectedBbanLength, String
                    .format("[%s] length is %d, expected BBAN length is: %d", bban, bbanLength, expectedBbanLength));
        }
    }
            ...
            case a:
                for (char ch : entryValue.toCharArray()) {
                    if (!Character.isUpperCase(ch)) {
                        throw new IbanFormatException(BBAN_ONLY_UPPER_CASE_LETTERS, entry.getEntryType(), entryValue,
                                ch,
            ...
            case c:
                for (char ch : entryValue.toCharArray()) {
                    if (!Character.isLetterOrDigit(ch)) {
                        throw new IbanFormatException(BBAN_ONLY_DIGITS_OR_LETTERS, entry.getEntryType(), entryValue, ch,
            ...
            case n:
                for (char ch : entryValue.toCharArray()) {
                    if (!Character.isDigit(ch)) {
                        throw new IbanFormatException(BBAN_ONLY_DIGITS, entry.getEntryType(), entryValue, ch,
    ...
    private static int calculateMod(final String iban) {
        ...
        for (int i = 0; i < reformattedIban.length(); i++) {
            ...
            if (numericValue < 0 || numericValue > 35) {
                throw new IbanFormatException(IBAN_VALID_CHARACTERS, null, null, reformattedIban.charAt(i),
