...
public class TransactionsMFAFlowTestPreProcessor implements ObjectServicePreProcessor {
	private static  Logger LOG = LogManager.getLogger(TransactionsMFAFlowTestPreProcessor.class);
	...
	private JsonObject getSecurityQuestionsRequestPayload(JsonObject responseFromTransfer,
		...
		mfaAttr.addProperty(MFAConstants.SERVICE_NAME, requestPayload.get(MFAConstants.MFA_ATTRIBUTES).getAsJsonObject().get(MFAConstants.SERVICE_NAME).getAsString());
	...
	private JsonObject getverifyOTPPayload(JsonObject responseFromTransfer, JsonObject requestPayload,
		...
		if (payloadforverifyOTP == null) {
			...
			mfaAttr.addProperty(MFAConstants.SERVICE_NAME, requestPayload.get(MFAConstants.MFA_ATTRIBUTES)
					.getAsJsonObject().get(MFAConstants.SERVICE_NAME).getAsString());
			if (mfaAttributes.has(MFAConstants.SERVICE_KEY)) {
