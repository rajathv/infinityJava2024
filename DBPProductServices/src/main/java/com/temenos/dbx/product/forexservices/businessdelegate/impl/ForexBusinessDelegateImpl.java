...
public class ForexBusinessDelegateImpl implements ForexBusinessDelegate {
	...
	public List<CurrencyDTO> getPopularCurrencies(String baseCurrencyCode, DataControllerRequest dcRequest) {
	    ...
	    filter.append(SqlQueryEnum.valueOf(jdbcUrl + "_Currency_Get").getQuery().replace("?1",schema).replace("?2",baseCurrencyCode));
	...
	public List<CurrencyDTO> getRecentCurrencies(String customerId, DataControllerRequest dcRequest) {
		...
		if (StringUtils.isBlank(legalEntityId))
			filter.append(SqlQueryEnum.valueOf(jdbcUrl + "_CurrencyRecentCurrency_Get").getQuery().replace("?1", schema)
					.replace("?2", customerId));
		else
			filter.append(SqlQueryEnum.valueOf(jdbcUrl + "_CurrencyRecentCurrency_Get_LegalEntityId").getQuery().replace("?1", schema)
					.replace("?2", customerId).replace("?3", legalEntityId));
