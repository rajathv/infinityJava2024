...
public class FetchAllMyApprovalHistoryOperation implements JavaService2{
	
	private static final Logger LOG = LogManager.getLogger(FetchAllMyApprovalHistoryOperation.class);
	...
	public Object invoke(String methodId, Object[] inputArray, DataControllerRequest request, DataControllerResponse response) {
		...
		try {
			...
			result = approvalQueueResource.fetchMyApprovalHistory(methodId, inputArray, request, response);
		}
		catch(Exception e) {
			LOG.error("Error occured while invoking fetchMyApprovalHistory: ", e);
