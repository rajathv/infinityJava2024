...
public class CreateTransactionOperation implements JavaService2 {
	private static final Logger LOG = LogManager.getLogger(CreateTransactionOperation.class);
	...
	public Object invoke(String methodID, Object[] inputArray, DataControllerRequest request, DataControllerResponse response) {
		...
		try {
	         result = achTransactionResource.createACHTransaction(methodID, inputArray, request, response);			
		}
		catch(Exception e) {
			LOG.error("Error occured while invoking createACHTransaction: ", e);
