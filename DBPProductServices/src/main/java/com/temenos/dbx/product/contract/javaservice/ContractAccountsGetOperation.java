...
public class ContractAccountsGetOperation implements JavaService2 {
    LoggerUtil logger = new LoggerUtil(ContractAccountsGetOperation.class);
    ...
    public Object invoke(String methodId, Object[] inputArray, DataControllerRequest request,
        ...
        try {
            ...
            result = resource.getContractAccounts(methodId, inputArray, request, response);
        } catch (ApplicationException e) {
            e.getErrorCodeEnum().setErrorCode(result);
            logger.error("Exception occured while fetching contract accounts " + e.getMessage());
        } catch (Exception e) {
            logger.error("Exception occured while fetching contract accounts " + e.getMessage());
            ErrorCodeEnum.ERR_10784.setErrorCode(result);
        }
        return result;
    }
