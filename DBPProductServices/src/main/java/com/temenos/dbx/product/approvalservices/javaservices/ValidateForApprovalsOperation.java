...
public class ValidateForApprovalsOperation implements JavaService2{
	
	private static final Logger LOG = LogManager.getLogger(ValidateForApprovalsOperation.class);
	...
	public Object invoke(String methodId, Object[] inputArray, DataControllerRequest request, DataControllerResponse response) {
		...
		try {
			...
			result = approvalQueueResource.validateForApprovals(methodId, inputArray, request, response);
		}
		catch(Exception e) {
			LOG.error("Error occured while invoking GetApprovalStatusOperation: ", e);
