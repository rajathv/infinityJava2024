...
public class InternationalWireTransferResourceImpl implements InternationalWireTransferResource {

	private static final Logger LOG = LogManager.getLogger(InternationalWireTransferResourceImpl.class);
	...
	InternationalWireTransactionBusinessDelegate wireTransactionDelegate = DBPAPIAbstractFactoryImpl.getBusinessDelegate(InternationalWireTransactionBusinessDelegate.class);
	InternationalWireTransactionBackendDelegate wireTransactionBackendDelegate = DBPAPIAbstractFactoryImpl.getBackendDelegate(InternationalWireTransactionBackendDelegate.class);
	...
	public Result createTransaction(String methodID, Object[] inputArray, DataControllerRequest request, 
		...
		featureActionId = FeatureAction.INTERNATIONAL_WIRE_TRANSFER_CREATE;
		...
		if("true".equalsIgnoreCase(validate)) {
			...
			WireTransactionDTO validateAccountDTO = wireTransactionBackendDelegate.validateTransaction(wireBackendDTO, request);
			try {
				 result = JSONToResult.convert(new JSONObject(validateAccountDTO).toString());
				 return result;
			} catch (JSONException e) {
				LOG.error("Error occured while converting the response from Line of Business service for International Wire transfer: ", e);
		...
		try {
			String responseObj = new JSONObject(wireBackendDTO).toString();
			result = JSONToResult.convert(responseObj);
		} catch (JSONException e) {
			LOG.error("Error occured while converting the response from Line of Business service for Wire transfer: ", e);
			return ErrorCodeEnum.ERR_21217.setErrorCode(new Result());
		}
		...
		if(transactionStatus == TransactionStatusEnum.SENT ) {
			...
			//code snippet being added for alerts on International wire transfer
			try {
	...
	public Result updateStatus(String MethodID, Object[] inputArray,
		...
		try {
			...
			InternationalWireTransactionBusinessDelegate wireTransactionDelegate = 
    ...
    public Result processResponseFromLineOfBusiness(Result result, DataControllerRequest request,
    	...
    	InternationalWireTransactionBusinessDelegate wireBusinessDelegate = DBPAPIAbstractFactoryImpl.getInstance()
