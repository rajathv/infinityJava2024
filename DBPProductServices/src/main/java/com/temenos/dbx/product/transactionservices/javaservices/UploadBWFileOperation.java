...
public class UploadBWFileOperation implements JavaService2 {
	...
	public Object invoke(String methodID, Object[] inputArray, DataControllerRequest request, DataControllerResponse response)
	{
		...
		try {
			BulkWireFileResource uploadBWfiles = DBPAPIAbstractFactoryImpl.getInstance().getFactoryInstance(ResourceFactory.class).getResource(BulkWireFileResource.class);
			result = uploadBWfiles.uploadBWFile(methodID, inputArray, request, response);
		}
		catch(Exception exp) {
