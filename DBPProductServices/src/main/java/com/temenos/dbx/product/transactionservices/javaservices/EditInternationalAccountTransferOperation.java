 ...
 */
public class EditInternationalAccountTransferOperation implements JavaService2{

	private static final Logger LOG = LogManager.getLogger(EditInternationalAccountTransferOperation.class);
	...
	public Object invoke(String methodID, Object[] inputArray, DataControllerRequest request, 
		...
		try {
			...
			result  = internationalTransactionResource.editTransaction(methodID, inputArray, request, response);
			...
			if(inputParams.containsKey("uploadedattachments") && inputParams.containsKey("validate")) {
				...
				if ((StringUtils.isBlank(isValidate)
					...
					if (StringUtils.isNotBlank(uploadedAttachments)) {
						...
						parseUploadingAttachments(request, result, referenceId, userId, uploadedAttachmentsArray);
					}
					if (StringUtils.isNotBlank(deletedAttachments)) {
						String deletedAttachmentsArray[] = deletedAttachments.split(",");
						deleteAttachments(request, result, referenceId, userId, deletedAttachmentsArray);
					}
				}
			}
		}
		catch(Exception e) {
			LOG.error("Error occured while invoking EditInternationalAccountTransferOperation: ", e);
