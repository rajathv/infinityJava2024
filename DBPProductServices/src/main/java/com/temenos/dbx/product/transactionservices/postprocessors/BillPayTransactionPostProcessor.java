...
public class BillPayTransactionPostProcessor implements DataPostProcessor2 {
	
	private static final Logger LOG = LogManager.getLogger(BillPayTransactionPostProcessor.class);
	...
	public Object execute(Result result, DataControllerRequest request, DataControllerResponse response)
		...
		try {
			...
			result  = billpayTranscationResource.processResponseFromLineOfBusiness(result, request, response);
		}
		catch(Exception e) {
			LOG.error("Error occured while invoking post processor for CreateBillPayTransaction: ", e);
