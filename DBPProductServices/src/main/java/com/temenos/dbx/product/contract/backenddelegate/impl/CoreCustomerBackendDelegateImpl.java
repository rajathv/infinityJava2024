...
public class CoreCustomerBackendDelegateImpl implements CoreCustomerBackendDelegate {
    ...
    private JsonObject searchCoreCustomerT24(Map<String, Object> procParams, Map<String, Object> headersMap) {
        addT24Headers(headersMap, (String) procParams.get("_id"));
    ...
    private JsonObject getCoreRelativeCustomerT24(MembershipDTO membershipDTO, Map<String, Object> headersMap) {
        JsonObject response = new JsonObject();
        JsonArray records = new JsonArray();
        addT24Headers(headersMap, membershipDTO.getId());
    ...
    private JsonObject getCoreCustomerAccountsArrangements(MembershipDTO membershipDTO,
        ...
        Map<String, Object> inputParams = new HashMap<>();
        if(membershipDTO.getCompanyLegalUnit().isEmpty()) {
        	membershipDTO.setCompanyLegalUnit(EnvironmentConfigurationsHandler.getValue(DBPUtilitiesConstants.BRANCH_ID_REFERENCE));
        }
        inputParams.put("coreCustomerId",
        		membershipDTO.getCompanyLegalUnit() + "-"
                        + membershipDTO.getId());
    ...
    private void addT24Headers(Map<String, Object> headersMap, String id) {
        ...
        if (headersMap.get("companyId") != null && StringUtils.isBlank(headersMap.get("companyId").toString())) {
            if (backendIdentifierDTO != null && StringUtils.isNotBlank(backendIdentifierDTO.getCompanyId())) {
                headersMap.put("companyId", backendIdentifierDTO.getCompanyId());
            } else {
    ...
    public List<AllAccountsViewDTO> getCoreCustomerAccounts(List<MembershipDTO> membershipList,
        ...
        try {
            ...
            JsonObject response = new JsonObject();
            final String IS_Integrated = Boolean.toString(IntegrationTemplateURLFinder.isIntegrated);
            if (StringUtils.isNotBlank(EnvironmentConfigurationsHandler
                    .getServerProperty("ARRANGEMENTS_HOST_URL"))) {
                JsonArray coreCustomerAccounts = new JsonArray();
