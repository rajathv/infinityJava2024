...
public class UpdateBackendIdInApprovalQueueOperation implements JavaService2{
	
	private static final Logger LOG = LogManager.getLogger(UpdateBackendIdInApprovalQueueOperation.class);
	...
	public Object invoke(String methodId, Object[] inputArray, DataControllerRequest request, DataControllerResponse response) {
		...
		try {
			...
			result = approvalQueueResource.updateBackendIdInApprovalQueue(methodId, inputArray, request, response);
		}
		catch(Exception e) {
			LOG.error("Error occured while invoking UpdateBackendIdInApprovalQueueOperation : ", e);
