...
public class CardUpdateMFAPreProcessor implements ObjectServicePreProcessor, ObjectProcessorTask {
    private static final Logger LOG = LogManager.getLogger(CardUpdateMFAPreProcessor.class);
    ...
    private String getServiceName(FabricRequestManager requestManager) {
        ...
        try {
            operationData = requestManager.getServicesManager().getOperationData();
            operationName = operationData.getOperationId();
			objectName = operationData.getObjectId();
        } catch (AppRegistryException e) {
            LOG.error("Exception occured while executing MFA preprocessor for card update", e);
        }

        if (StringUtils.isNotBlank(objectName+"_"+operationName)) {
            switch (objectName+"_"+operationName) {
                case "LockCard_createRequest":
                    serviceName = "CARD_MANAGEMENT_LOCK_CARD";
                    break;
                case "UnlockCard_createRequest":
                    serviceName = "CARD_MANAGEMENT_UNLOCK_CARD";
                    break;
                case "ReplaceCard_createRequest":
                    serviceName = "CARD_MANAGEMENT_REPLACE_CARD";
                    break;
                case "ReportLost_createRequest":
                    serviceName = "CARD_MANAGEMENT_REPORT_CARD_STOLEN";
                    break;
                case "ChangePIN_createRequest":
                    serviceName = "CARD_MANAGEMENT_CHANGE_PIN";
                    break;
                case "CancelCard_createRequest":
                    serviceName = "CARD_MANAGEMENT_CANCEL_CARD";
                    break;
                case "Cards_partialupdate":
                    serviceName = "CARD_MANAGEMENT_PARTIAL_UPDATE";
                    break;
				case "Cards_createCardRequest":
                case "ChangePIN_updateCreditCardPin":
                    serviceName = "CARD_MANAGEMENT_CREATE_CARD_REQUEST";
                    break;
			    case "ActivateCard_createRequest":
