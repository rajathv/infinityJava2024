...
public class VerifyPreLoginOTPPreProcessor implements ObjectServicePreProcessor {
    private static final Logger LOG = LogManager.getLogger(VerifyPreLoginOTPPreProcessor.class);
    ...
    public void execute(FabricRequestManager requestManager, FabricResponseManager responseManager,
        ...
        if (!mfaUtil.isAppActionValid()) {
            ErrorCodeEnum.ERR_10547.setErrorCode(resultJson, mfaUtil.getDbpErrMsg());
            responsePayloadHandler.updatePayloadAsJson(resultJson);
            return;
        }

        if (StringUtils.isBlank(serviceKey) || !mfaUtil.isValidOnlyServiceKey(serviceName)) {
        ...
        if (MFAtype.equals(MFAConstants.SECURE_ACCESS_CODE)) {
            if (null != mfaAttributes && mfaAttributes.has("OTP") && mfaAttributes.get("OTP").isJsonObject()) {
                changePayload(mfaAttributes.get("OTP").getAsJsonObject(), serviceKey, serviceName, mfaUtil,
        ...
        ErrorCodeEnum.ERR_10549.setErrorCode(resultJson);
        responsePayloadHandler.updatePayloadAsJson(resultJson);
        return;
    }

    private void changePayload(JsonObject OTP, String serviceKey, String serviceName, PreLoginUtil mfaUtil,
        ...
        String userId = mfaUtil.getMFAUserId();
