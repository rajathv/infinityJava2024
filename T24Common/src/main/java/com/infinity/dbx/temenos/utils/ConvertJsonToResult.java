...
public class ConvertJsonToResult {
	...
	public static Result convert(JsonObject jsonObject) {
		...
		if(null != jsonObject){
			for (Map.Entry<String, JsonElement> entry : 
				jsonObject.entrySet()) {
				processJsonElement(result,entry.getValue(),
						entry.getKey());
			}
		}else{
			result.addParam(new Param("errormsg",
					"Received empty response","String"));
		}
		return result;
	}
	
	private static void processJsonElement(Result result, 
			JsonElement jElement, String key){
		if(null == jElement){
			return;
	...
	private static void addRecordsToResult(Result result, 
	...
	private static void addDatasetToResult(Result result,JsonArray
	...
	private static void addRecordToRecord(Record record,JsonObject
			jsonArray, String key ){
	...
	private static void addDatasetToRecord(Record record,JsonArray
			jsonArray, String key ){
		Dataset ds = constructDataset(jsonArray);
		ds.setId(key);
	...
	private static Record constructRecord(JsonObject jsonObject){
		Record record = new Record();
		if(null != jsonObject){
			for (Map.Entry<String, JsonElement> entry : 
				jsonObject.entrySet()) {
				processJsonElement(record, entry.getValue(),
						entry.getKey());
			}
		}
		return record;
	...
	private static void processJsonElement(Record record, 
			JsonElement jElement, String key) {
		if(null == jElement){
			return;
		}else if(jElement.isJsonArray()){
