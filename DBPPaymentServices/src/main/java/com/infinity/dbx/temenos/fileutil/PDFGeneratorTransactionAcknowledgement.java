...
public class PDFGeneratorTransactionAcknowledgement {


	private Map<String, Object> transactionFields;
	private static final Logger log = LogManager.getLogger(PDFGeneratorTransactionAcknowledgement.class);

	public PDFGeneratorTransactionAcknowledgement(Map<String, Object> transactionFields) {
	...
	public String generateFileAndGetBase64() {
		try {
			...
			//adding header
			addHeaderLabel(document, "Transaction Details");
	...
	private PdfPTable getTable() {
		...
		for (Map.Entry<String, Object> entry : this.transactionFields.entrySet()) {
			try {
