...
public class T24EditTransactionWithApprover implements JavaService2 {

	private static final Logger LOG = LogManager.getLogger(T24EditTransactionWithApprover.class);
	...
	public Object invoke(String methodID, Object[] inputArray, DataControllerRequest request,
		...
		if (Constants.FREQUENCY_ONCE.equalsIgnoreCase(frequency)) {
			...
			return editRecurringTransactionWithApproval(requestparameters, request);
		}
	}
	...
	private static Result editRecurringTransactionWithApproval(Map<String, Object> requestParameters,
			DataControllerRequest request) {
		Result result = new Result();
		try {
			LOG.error("INPUT to BACKEND: " + new JSONObject(requestParameters).toString());
			String response = DBPServiceExecutorBuilder.builder()
					...
					.withOperationId(TemenosConstants.OP_EDIT_STANDINGORDER_WITH_APPROVER)
					.withRequestParameters(requestParameters).withRequestHeaders(request.getHeaderMap())
					.withDataControllerRequest(request).build().getResponse();
			result = JSONToResult.convert(response);
			return result;
