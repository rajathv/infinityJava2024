...
public class RequestPreLoginEnrollOTP implements JavaService2 {
    private static final Logger LOG = LogManager.getLogger(RequestPreLoginEnrollOTP.class);
    ...
    public Object invoke(String methodID, Object[] inputArray, DataControllerRequest dcRequest,
        ...
        if (!mfaUtil.isValidMFA()) {
            ErrorCodeEnum.ERR_10601.setErrorCode(result);
            return result;
        }

        if (!validate(dcRequest)) {
            result = new Result();
            ErrorCodeEnum.ERR_10602.setErrorCode(result);
            return result;
        }
        ...
        if (phoneOTPcount >= allowedLimit || emailOTPcount >= allowedLimit) {
            result = new Result();
            ErrorCodeEnum.ERR_10603.setErrorCode(result);
            return result;
        }
    ...
    private Result createOTP(String methodID, Object[] inputArray, DataControllerRequest dcRequest,
            ...
            String userId, PreLoginSMEnrollUtil mfaUtil) {
    ...
    private int generateOtp(PreLoginSMEnrollUtil mfaUtil) {
