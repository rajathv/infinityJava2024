...
public class VerifyOTPPreLoginEnroll implements JavaService2 {
    ...
    public Object invoke(String methodID, Object[] inputArray, DataControllerRequest dcRequest,
        ...
        mfaUtil = new PreLoginSMEnrollUtil(dcRequest);

        if (!mfaUtil.isValidMFA()) {
            ErrorCodeEnum.ERR_10596.setErrorCode(result);
            return result;
        }
    ...
    private Result postProcess(Map<String, String> inputParams, DataControllerRequest dcRequest, Result result,
            String otpBypass, PreLoginSMEnrollUtil mfaUtil) throws HttpCallException {
        ...
        if (HelperMethods.hasRecords(result)) {
            if (!isAttemptAllowed(dcRequest, result, mfaUtil)) {
                ...
                ErrorCodeEnum.ERR_10597.setErrorCode(retRes);
                ...
                ErrorCodeEnum.ERR_10598.setErrorCode(retRes);
                updateAttemptCount(dcRequest, result, retRes, mfaUtil);
                return mfaUtil.addVerifyAttributes(result, retRes);
            }
        } else {
            ErrorCodeEnum.ERR_10599.setErrorCode(retRes);
        }
    ...
    private void updateAttemptCount(DataControllerRequest dcRequest, Result result, Result retRes,
            PreLoginSMEnrollUtil mfaUtil) throws HttpCallException {
    ...
    private void setVerifyAttributes(DataControllerRequest dcRequest, Result result, Result retRes,
        ...
        if (attemptsLeft <= 0) {
            ...
            ErrorCodeEnum.ERR_10600.setErrorCode(retRes);
        }
    }

    private boolean isAttemptAllowed(DataControllerRequest dcRequest, Result result, PreLoginSMEnrollUtil mfaUtil) {
        String str = HelperMethods.getFieldValue(result, "InvalidAttempt");
        if (StringUtils.isNotBlank(str)) {
            int allowedAttempt = mfaUtil.maxFailedAttemptsAllowed();
    ...
    private boolean isOTPExpired(DataControllerRequest dcRequest, Result result, PreLoginSMEnrollUtil mfaUtil) {
