...
public class ActivateQRPaymentForUser implements JavaService2 {
	private static final Logger LOG = LogManager.getLogger(ActivateQRPaymentForUser.class);
	@Override
	...
	private boolean preProcess(Map<String, String> inputParams, DataControllerRequest dcRequest, Result result) {
		...
		if (status && checkRecordExists(inputParams, dcRequest, result)) {
			if (status) {
				...
				inputParams.put(DBPUtilitiesConstants.IS_QR_PAYMENT_ACTIVE, "true");
