...
import com.kony.memorymgmt.EventLogManager;
 ...
 *
 */
public class EventLogUtils implements ObjectServicesConstants {
    ...
    public JsonObject buildEventData(FabricRequestManager fabricRequestManager, JsonObject request,
        ...
        if (response != null && !response.entrySet().isEmpty()) {
            if (ObjectServiceHelperMethods.hasKey(response, PARAM_OP_STATUS)) {
                opstatus = HelperMethods.getStringFromJsonObject(response, PARAM_OP_STATUS, true);
            }

            if (opstatus.equals("0") && !ObjectServiceHelperMethods.hasKey(response, PARAM_DBP_ERR_CODE)) {
    ...
    public JsonObject LoadEventLogProperties(FabricRequestManager fabricRequestManager) throws FileNotFoundException {
        ...
        EventLogManager EventLogManager = new EventLogManager(fabricRequestManager);
        String customerId = HelperMethods.getCustomerIdFromSession(fabricRequestManager);
        final String CACHE_KEY_AUDIT_PROPERTIES = "EventLogProperties";
        JsonObject EventLogProperties = new JsonObject();
        SessionMap EventLogMapProperties = EventLogManager.getAuditPropertiesFromSession(customerId);

        if (null != EventLogMapProperties && !EventLogMapProperties.isEmpty()) {
            LOG.error("Audit Log properties found in cache");
            if (EventLogMapProperties.hasKey(CACHE_KEY_AUDIT_PROPERTIES)) {
                String CachedProperties = EventLogMapProperties.getAttributeValueForKey(CACHE_KEY_AUDIT_PROPERTIES,
                        CACHE_KEY_AUDIT_PROPERTIES);
                EventLogProperties = new Gson().fromJson(CachedProperties, JsonObject.class);
                return EventLogProperties;
            }
        }

        InputStream inputStream = EventLogUtils.class.getClassLoader().getResourceAsStream("EventLog.json");
        ...
        // Storing properties in cache when flag is enabled
        if (EventLogProperties != null && !EventLogProperties.entrySet().isEmpty()) {
            if (EventLogProperties.has(PARAM_STORE_IN_CACHE)) {
                String StoreInCache = EventLogProperties.get(PARAM_STORE_IN_CACHE).getAsString();
                if (StoreInCache.equalsIgnoreCase(PARAM_TRUE)) {
                    SessionMap EventLogMap = new SessionMap();
                    EventLogMap.addAttributeForKey(CACHE_KEY_AUDIT_PROPERTIES, CACHE_KEY_AUDIT_PROPERTIES,
                            EventLogProperties.toString());
                    EventLogManager.saveAuditPropertiesIntoSession(EventLogMap);
                }
            }
        }
