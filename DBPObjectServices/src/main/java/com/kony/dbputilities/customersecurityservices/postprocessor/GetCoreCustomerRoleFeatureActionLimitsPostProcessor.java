...
public class GetCoreCustomerRoleFeatureActionLimitsPostProcessor implements ObjectServicePostProcessor {
	private static final Logger LOG = LogManager.getLogger(GetCoreCustomerRoleFeatureActionLimitsPostProcessor.class);
	...
	public void execute(FabricRequestManager requestManager, FabricResponseManager responseManager) throws Exception {
		...
		if (jsonObject.has(InfinityConstants.limits) && !jsonObject.get(InfinityConstants.limits).isJsonNull()) {
			...
			for (JsonElement element : limits) {
				...
				if (company.has(InfinityConstants.coreCustomerLimits)
						&& !company.get(InfinityConstants.coreCustomerLimits).isJsonNull()) {
					JsonArray contractCustomerLimits = company.get(InfinityConstants.coreCustomerLimits)
							.getAsJsonArray();
