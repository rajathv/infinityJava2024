 ...
 *
 */
public class SaveBulkWireFileIdsInSessionTask implements ObjectProcessorTask {
    ...
    public boolean process(FabricRequestManager fabricRequestManager, FabricResponseManager fabricResponseManager)
        ...
        try {
            ...
            if (null != response && !response.isJsonNull()) {
                if (response.has("BulkWireFiles")) {
                    JsonArray BulkWires = response.getAsJsonArray("BulkWireFiles");
                ...
                payeemanager.saveBulkWireFileIdsIntoSession(BulkWiresMap);
            }
        } catch (Exception e) {
    ...
    private SessionMap getPayeesMap(JsonArray payees) {
        ...
        if (null != payees && !payees.isJsonNull() && payees.size() > 0) {
            ...
            while (itr.hasNext()) {
                ...
                payeesMap.addKey(payee.get("bulkWireFileID").getAsString());
            }
        }
        return payeesMap;
    }
