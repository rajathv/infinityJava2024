...
public class GetPendingDepositTransactions implements JavaService2 {
    ...
    private boolean preProcess(Map inputParams, DataControllerRequest dcRequest, Result result) {
        ...
        if (HelperMethods.isBusinessUserType(user.get("customerType"))) {
			 ...
			 */
        	filter.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetPendingDepositTransactions_ByCustomerId").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", user.get("customer_id")));
        } else {
			/*
			 * filter.append("select t1.*,tt.description as transactionType from " +
			 * URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest) +
			 ...
			 * filter.append(" ORDER BY t1.createdDate desc limit 0, 10");
			 */
        	filter.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetPendingDepositTransactions_ByUserId").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", user.get("customer_id")));
        }
