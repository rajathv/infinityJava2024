...
public class GetAccountPendingTransactionsPreview implements JavaService2 {
    ...
    private boolean preProcess(Map inputParams, DataControllerRequest dcRequest, Result result)
		 ...
		 * queryBuf.append(" and (transaction.StatusDesc = 'Pending'"); queryBuf.
		 * append(" or transaction.cashWithdrawalTransactionStatus = 'Pending')");
		 * queryBuf.append(") ORDER BY " + sortBy + " " + order);
		 */
        queryBuf.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetAccountPendingTransactionsPreview").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", accountId).replace("?3", sortBy).replace("?4", order));
