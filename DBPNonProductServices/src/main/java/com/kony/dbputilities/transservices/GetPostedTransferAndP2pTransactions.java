...
public class GetPostedTransferAndP2pTransactions implements JavaService2 {
    ...
    private boolean preProcess(Map inputParams, DataControllerRequest dcRequest, Result result,
        ...
        filter.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetTransactionTypeDetails_ByCustomerId").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", user.get("user_id")));
        ...
        /*if (HelperMethods.isBusinessUserType(user.get("customerType"))) {
			 ...
			 * append("where t.isScheduled = 0 and (t.FromAccountNumber in (select account_id from "
			 * + URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest) +
        	...
        	filter.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetTransactionTypeDetails_ByUserId").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", user.get("user_id")));
        }*/
