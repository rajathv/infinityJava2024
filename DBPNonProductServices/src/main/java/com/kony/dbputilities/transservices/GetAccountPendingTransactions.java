...
public class GetAccountPendingTransactions implements JavaService2 {
	...
	private void updateDateFormat(Record transaction) {
        try {
            ...
            HelperMethods.updateDateFormat(transaction, "postedDate");
    ...
    private void updateTotalChkAmount(Record transaction) {
        ...
        String total = "0";
        if (StringUtils.isNotBlank(amt3) && StringUtils.isNotBlank(amt2) && StringUtils.isNotBlank(amt1)) {
    ...
    private boolean preProcess(Map inputParams, DataControllerRequest dcRequest, Result result)
        ...
        /*
         * sb.append("(").append(DBPUtilitiesConstants.FRM_ACCONT)
         * .append(DBPUtilitiesConstants.EQUAL).append(accountId); sb.append(DBPUtilitiesConstants.OR);
         * sb.append(DBPUtilitiesConstants.TO_ACCONT) .append(DBPUtilitiesConstants.EQUAL).append(accountId)
         * .append(")"); sb.append(DBPUtilitiesConstants.AND).append("(");
         * sb.append("StatusDesc").append(DBPUtilitiesConstants.EQUAL) .append("Pending");
         * sb.append(DBPUtilitiesConstants.OR); sb.append("cashWithdrawalTransactionStatus")
         * .append(DBPUtilitiesConstants.EQUAL).append("Pending") .append(")"); sb.append(DBPUtilitiesConstants.AND);
         * sb.append("("); sb.append("isScheduled").append(DBPUtilitiesConstants.EQUAL) .append("false");
         * sb.append(DBPUtilitiesConstants.OR); sb.append("isScheduled").append(DBPUtilitiesConstants.EQUAL)
         * .append("0"); sb.append(")");
         */
		/*
		 * queryBuf.
		 ...
		 * queryBuf.append(" and (transaction.statusDesc = 'Pending')");
		 * queryBuf.append(") ORDER BY " + sortBy + " " + order);
		 */
        queryBuf.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetAccountPendingTransactions").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", accountId).replace("?3", sortBy).replace("?4", order));
        ...
        /*
         * String id = HelperMethods.getCustomerIdFromSession(dcRequest); if(StringUtils.isNotBlank(id)) {
         * sb.append(DBPUtilitiesConstants.AND); sb.append("User_id").append(DBPUtilitiesConstants.EQUAL).append(id); }
         */

        /*
         * if (StringUtils.isBlank(sortBy)) { sortBy = "transactionDate"; } if (StringUtils.isBlank(order)) { order =
         * "desc"; } if (StringUtils.isNotBlank(offset)) { inputParams.put(DBPUtilitiesConstants.SKIP, offset); } if
         * (StringUtils.isNotBlank(limit)) { inputParams.put(DBPUtilitiesConstants.TOP, limit); }
         * inputParams.put(DBPUtilitiesConstants.FILTER, sb.toString()); inputParams.put(DBPUtilitiesConstants.ORDERBY,
         * sortBy+ " " + order);
         */
