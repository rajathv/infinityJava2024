...
public class GetPendingCardlessTransactions implements JavaService2 {
    private static final Logger LOG = LogManager.getLogger(GetPendingCardlessTransactions.class);
    ...
    private boolean preProcess(Map inputParams, DataControllerRequest dcRequest, Result result) {
        ...
        if (HelperMethods.isBusinessUserType(user.get("customerType"))) {
        	filter.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetPendingCardlessTransactions_ByCustomerId").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", user.get("customer_id")).replace("?3", legalEntityId));
        } else {
        	filter.append(SqlQueryEnum.valueOf(jdbcUrl+"_GetPendingCardlessTransactions_ByUserId").getQuery().replace("?1", URLFinder.getPathUrl(URLConstants.SCHEMA_NAME, dcRequest)).replace("?2", user.get("customer_id")).replace("?3", legalEntityId));
        }
