...
public class BulkPaymentTemplateResourceImpl implements BulkPaymentTemplateResource {
	...
	public Result createBulkPaymentTemplate(String methodID, Object[] inputArray, DataControllerRequest dcRequest, 
		...
		try {
			...
			responseDTO = bulkPaymentTemplateBusinessDelegate.createTemplate(bulkPaymentTemplateDTO, legalEntityId);
	...
	private BulkPaymentTemplateDTO _validateRequestParamsForCreateTemplate(HashMap<String, Object> requestMap, Map<String, Object> customer) {
		...
		if (StringUtils.isEmpty(bulkPaymentTemplateDTO.getTemplateName()) || !bulkPaymentTemplateDTO.getTemplateName().matches("^[[a-zA-Z0-9]\\s?.:)(+_\\/]*")) {
			LOG.error("Invalid Template Name");
			bulkPaymentTemplateDTO.setDbpErrorCode(ErrorCodeEnum.ERR_28001);
			return bulkPaymentTemplateDTO; 
		}
		
		if (StringUtils.isEmpty(bulkPaymentTemplateDTO.getDescription()) || !bulkPaymentTemplateDTO.getDescription().matches("^[[a-zA-Z0-9]\\s?.,':)(+-\\/]*") || bulkPaymentTemplateDTO.getDescription().length() > 35) {
			LOG.error("Invalid Description");
			bulkPaymentTemplateDTO.setDbpErrorCode(ErrorCodeEnum.ERR_21241);
			return bulkPaymentTemplateDTO; 
		}
		...
		JSONArray paymentRef = new JSONArray(requestMap.get("POs").toString());
		for (int i=0 ; i < paymentRef.length(); i++)
		 {
		JSONObject obj = (JSONObject) paymentRef.get(i);
		String paymentReference = obj.get("paymentReference").toString();
		if(StringUtils.isEmpty (paymentReference) || !paymentReference.matches("^[[a-zA-Z0-9]\\s?.,':)(+-\\/]*") || paymentReference.length() > 35) {
		LOG.error("Invalid Description");
		bulkPaymentTemplateDTO.setDbpErrorCode(ErrorCodeEnum.ERR_21241);
		return bulkPaymentTemplateDTO;
		 } 
	...
	public Result fetchBulkPaymentTemplates(String methodID, Object[] inputArray, DataControllerRequest request,
		...
		List<BulkPaymentTemplateDTO> templates = bulkPaymentTemplateBusinessDelegate.fetchTemplates(customerId, contracts, legalEntityId);
	...
	public Result deleteBulkPaymentTemplate(String methodId, Object[] inputArray, DataControllerRequest request,
		...
		BulkPaymentTemplateDTO filteredTemplate = bulkPaymentTemplateBusinessDelegate.fetchTemplateById(templateId, legalEntityId);
	...
	public Result editBulkPaymentTemplate(String methodId, Object[] inputArray, DataControllerRequest request,
        ...
        String previousAccount = bulkPaymentTemplateBusinessDelegate.fetchTemplateById(templateId,legalEntityId).getFromAccount();
        ...
        if(isTemplateDeleted) {
        	...
        	updatedTemplateDTO = bulkPaymentTemplateBusinessDelegate.createTemplate(bulkPaymentTemplateDTO, legalEntityId);
	...
	public Result createBulkRequest(String methodID, Object[] inputArray, DataControllerRequest dcRequest,
		...
		try {
			...
			String executionDate = inputParams.get("executionDate") != null ? inputParams.get("executionDate").toString() : null;
			...
			BulkPaymentTemplateDTO template = bulkPaymentTemplateBusinessDelegate.fetchTemplateById(templateId,legalEntityId);
			
			if (template != null) {
				double amount = 0.00;
				for(int i = 0; i<poInputList.size(); i++) {
					if(poInputList.get(i).getAmount() != null) {
						amount = amount + Double.parseDouble(poInputList.get(i).getAmount()); 
					}
				}
				if(amount != 0.00) {
					if(amount != template.getTotalAmount()) {
						return ErrorCodeEnum.ERR_21234.setErrorCode(new Result());
					}
				}
							
				List<BulkPaymentTemplatePODTO> paymentOrders = bulkPaymentTemplateBusinessDelegate.fetchTemplatePOsByTemplateId(templateId,legalEntityId);
				...
				if (paymentOrders.size() > 0) {
					...
					if(StringUtils.isNotBlank(executionDate)) 
						template.setPaymentDate(executionDate);
					
					BulkPaymentTemplateDTO requestDTO =  bulkPaymentTemplateBusinessDelegate.createTemplateRequest(template, paymentOrders, legalEntityId);
					
					if(requestDTO != null) {
						
					String encodedContent = bulkPaymentTemplateBusinessDelegate.generateCSV(requestDTO.getPaymentrequestId(), legalEntityId);
