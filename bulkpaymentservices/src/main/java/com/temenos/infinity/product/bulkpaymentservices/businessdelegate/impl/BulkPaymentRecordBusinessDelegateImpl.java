...
public class BulkPaymentRecordBusinessDelegateImpl implements BulkPaymentRecordBusinessDelegate {
   ...
   public List<CustomerActionDTO> fetchPermissionFromDBX(Set<String> accountIds){
	   String serviceName = ServiceId.DBPRBLOCALSERVICEDB;
	   String operationName = OperationName.DB_GET_CUSTOMERACTIONS;
	   Map<String, Object> requestParameters = new HashMap<>();
	   List<CustomerActionDTO> customerActionDtoList = null;


	   String filter = "";
	   for (String accounts : accountIds) {
		   if(filter.isEmpty())
			   filter = "Account_id" + DBPUtilitiesConstants.EQUAL + accounts;
		   else
			   filter = filter +  DBPUtilitiesConstants.OR +"Account_id" + DBPUtilitiesConstants.EQUAL + accounts;
	   }
	   filter= DBPUtilitiesConstants.OPEN_BRACE+filter+DBPUtilitiesConstants.CLOSE_BRACE+DBPUtilitiesConstants.AND + DBPUtilitiesConstants.OPEN_BRACE+
			   "Action_id"+DBPUtilitiesConstants.EQUAL+"BULK_PAYMENT_TEMPLATE_SINGLE_CREATE" +DBPUtilitiesConstants.OR+
			   "Action_id"+DBPUtilitiesConstants.EQUAL+"BULK_PAYMENT_TEMPLATE_MULTIPLE_CREATE"+DBPUtilitiesConstants.OR+
			   "Action_id"+DBPUtilitiesConstants.EQUAL+"BULK_PAYMENT_FILES_SINGLE_UPLOAD_CSV"+DBPUtilitiesConstants.OR+
			   "Action_id"+DBPUtilitiesConstants.EQUAL+"BULK_PAYMENT_FILES_SINGLE_UPLOAD_XML"+DBPUtilitiesConstants.OR+
			   "Action_id"+DBPUtilitiesConstants.EQUAL+"BULK_PAYMENT_FILES_MULTI_UPLOAD_CSV"+DBPUtilitiesConstants.OR+
			   "Action_id"+DBPUtilitiesConstants.EQUAL+"BULK_PAYMENT_FILES_MULTI_UPLOAD_XML"+ DBPUtilitiesConstants.CLOSE_BRACE;

	   String permissionResponse = null;
	   requestParameters.put(DBPUtilitiesConstants.FILTER, filter);
	   try {
		   permissionResponse = DBPServiceExecutorBuilder.builder().
				   withServiceId(serviceName).
				   withObjectId(null).
				   withOperationId(operationName).
				   withRequestParameters(requestParameters).
		   ...
		   //Replacing fields according to the fieldname in DTO for conversion
		   String permissionResponseAfterReplace=permissionResponse.replaceAll("Account_id","accountId")
				   .replaceAll("Action_id","actionId")
				   .replaceAll("RoleType_id","roleTpeId")
				   .replaceAll("Customer_id","customerId");

		   JSONObject responseObj = new JSONObject(permissionResponseAfterReplace);
		   JSONArray jsonArray = CommonUtils.getFirstOccuringArray(responseObj);
		   customerActionDtoList = JSONUtils.parseAsList(jsonArray.toString(), CustomerActionDTO.class);
	   }
	   catch (JSONException e) {
		   LOG.error("Failed to fetch Permissions from dbx: " + e);
		   return null;
	   }
	   catch (Exception e) {
		   LOG.error("Caught exception at fetching Permissions from dbx: " + e);
		   return null;
	   }
	   return customerActionDtoList;
   }
