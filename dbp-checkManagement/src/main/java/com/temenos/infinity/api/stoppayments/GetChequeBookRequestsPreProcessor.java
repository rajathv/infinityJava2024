...
public class GetChequeBookRequestsPreProcessor extends TemenosBasePreProcessor {
    ...
    public boolean execute(@SuppressWarnings("rawtypes") HashMap params, DataControllerRequest request,
        ...
        final Logger LOG = LogManager.getLogger(GetChequeBookRequestsPreProcessor.class);

        try {
        	LOG.error("getchequebookrequestspreprocRequest"+request);
        	LOG.error("getchequebookrequestspreprocRequestCompanyId"+request.getHeader("companyId"));
            LOG.error("getchequebookrequestspreprocparams"+params);
            super.execute(params, request, response, result);
        	LOG.error("getchequebookrequestspreprocRequestCompanyIdaftercheck"+request.getHeader("companyId"));
