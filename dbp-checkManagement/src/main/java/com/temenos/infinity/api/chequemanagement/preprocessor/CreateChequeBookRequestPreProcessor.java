...
public class CreateChequeBookRequestPreProcessor implements DataPreProcessor2{
	...
	public boolean execute(HashMap inputMap, DataControllerRequest request, DataControllerResponse response,
		...
		//we don't call validateForApprovals anymore, as Approvals MA might not be present in the environment
		...
		DataControllerRequest newRequest = request.clone();
		chequeBookDTO = chequeBookBusinessDelegate.validateForApprovals(chequeBookDTO, newRequest);
		if (chequeBookDTO == null) {
			...
			if (chequeBookDTO.getStatus().equalsIgnoreCase("ApprovalsMAMissing")) {
		        response.setStatusCode(200);
		        response.setAttribute(DBPConstants.FABRIC_OPSTATUS_KEY, 0);
				request.addRequestParam_("signatoryApprovalRequired", "false");
				return true;	
			}
		}
