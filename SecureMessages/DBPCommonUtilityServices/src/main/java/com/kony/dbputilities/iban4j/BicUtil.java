...
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.BANK_CODE_ONLY_LETTERS;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.BIC_LENGTH_8_OR_11;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.BIC_NOT_EMPTY;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.BIC_NOT_NULL;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.BIC_ONLY_UPPER_CASE_LETTERS;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.BRANCH_CODE_ONLY_LETTERS_OR_DIGITS;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.COUNTRY_CODE_ONLY_UPPER_CASE_LETTERS;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.LOCATION_CODE_ONLY_LETTERS_OR_DIGITS;
import static com.kony.dbputilities.iban4j.BicFormatException.BicFormatViolation.UNKNOWN;
...
public class BicUtil {
     ...
     *             if bic is invalid. UnsupportedCountryException if bic's country is not supported.
     */
    ...
    private static void validateBankCode(final String bic) {
        ...
        for (final char ch : bankCode.toCharArray()) {
            if (!Character.isLetter(ch)) {
                throw new BicFormatException(BANK_CODE_ONLY_LETTERS, ch, "Bank code must contain only letters.");
            }
        }
    }
    ...
    private static void validateCountryCode(final String bic) {
        ...
        if (countryCode.trim().length() < COUNTRY_CODE_LENGTH || !countryCode.equals(countryCode.toUpperCase())
                || !Character.isLetter(countryCode.charAt(0)) || !Character.isLetter(countryCode.charAt(1))) {
            throw new BicFormatException(COUNTRY_CODE_ONLY_UPPER_CASE_LETTERS, countryCode,
    ...
    private static void validateLocationCode(final String bic) {
        ...
        for (char ch : locationCode.toCharArray()) {
            if (!Character.isLetterOrDigit(ch)) {
                throw new BicFormatException(LOCATION_CODE_ONLY_LETTERS_OR_DIGITS, ch,
    ...
    private static void validateBranchCode(final String bic) {
        ...
        for (final char ch : branchCode.toCharArray()) {
            if (!Character.isLetterOrDigit(ch)) {
                throw new BicFormatException(BRANCH_CODE_ONLY_LETTERS_OR_DIGITS, ch,
