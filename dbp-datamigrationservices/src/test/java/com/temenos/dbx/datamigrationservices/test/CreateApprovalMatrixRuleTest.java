...
import com.temenos.dbx.datamigrationservices.javaservice.CreateApprovalMatrixRule;
...
public class CreateApprovalMatrixRuleTest {
	...
	Object[] inputArray;
	Result actualResult;
	MigrateApprovalMatrixResource migrationResource;
	...
	public void executedBefore() throws Exception {
		...
		migrationResource = mock(MigrateApprovalMatrixResource.class);
		...
		when(dbpAPIAbstractFactory
                .getFactoryInstance(ResourceFactory.class)).thenReturn(resourceFactory);
		when(resourceFactory.getResource(MigrateApprovalMatrixResource.class))
			.thenReturn(migrationResource);
		javaService2 = new CreateApprovalMatrixRule();
	}
	...
	public void testInvokeCreateContract() throws Exception {
		...
		when(migrationResource.createOrUpdateApprovalRule(methodID, inputArray, request, response)).thenReturn(result);
