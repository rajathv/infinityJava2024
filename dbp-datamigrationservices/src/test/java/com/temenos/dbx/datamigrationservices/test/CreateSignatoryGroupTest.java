...
import com.temenos.dbx.datamigrationservices.javaservice.CreateSignatoryGroup;
...
public class CreateSignatoryGroupTest {
	...
	public void executedBefore() throws Exception {
		...
		when(resourceFactory.getResource(MigrateInfinityUserResource.class)).thenReturn(migrationResource);
		javaService2 = new CreateSignatoryGroup();
	}

	@Test
	public void testInvokeLinkUserToContract() throws Exception {
		Result result = new Result();
		result.addOpstatusParam(0);
		result.addHttpStatusCodeParam(0);

		when(migrationResource.linkUserToContract(methodID, inputArray, request, response)).thenReturn(result);
