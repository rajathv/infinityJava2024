...
public class UpdateT24CustomerOperation implements JavaService2, AccountsConstants, TemenosConstants, UserConstants {

	private static final Logger logger = LogManager.getLogger(UpdateT24CustomerOperation.class);
	...
	public Object invoke(String methodId, Object[] inputArray, DataControllerRequest request,
		...
		try {
			...
			if (employments.size() > 0) {
				...
				if (StringUtils.isNotBlank(employmentObject.get("employerName").getAsString()))
					request.addRequestParam_("employerName", employmentObject.get("employerName").getAsString());
				if (StringUtils.isNotBlank(employmentObject.get("employerOfficePhone").getAsString()))
					request.addRequestParam_("employerOfficePhone",
							employmentObject.get("employerOfficePhone").getAsString());
				if (StringUtils.isNotBlank(employmentObject.get("employerOfficePhoneCode").getAsString())) {
				...
				if (occupationDetails.size() > 0) {
					...
					if (occupationObject.has("occupationType")&&StringUtils.isNotBlank(occupationObject.get("occupationType").getAsString())) {
			...
			String partyId = request.getParameter("partyID");
			 ...
			 if (StringUtils.isBlank(customerId)) {
			        result = new Result();
			        logger.debug("updateT24Operation is failed");
			...
			if (StringUtils.isNotBlank(entityName)) {
				...
				if (StringUtils.isNotBlank(dateOfIncorporation)) {
					...
					JsonObject registrationDetails = new JsonParser().parse(request.getParameter("registrationDetails"))
							.getAsJsonObject();
			...
			if (StringUtils.isNotBlank(relationship)) {
				...
				if (relationshipObj.has("cif") && StringUtils.isNotBlank(relationshipObj.get("cif").getAsString())) {
			...
			if(StringUtils.isBlank(coreCustomerId))
				coreCustomerId = getCoreCustomerId(customerId, request);
			...
			result = CommonUtils.callIntegrationService(request, null, headerMap, UserConstants.SERVICE_ID_ONBOARDING_USER,
					UserConstants.OP_UPDATE_T24, true);
		} catch (Exception exception) {
			logger.error("Error in updatet24Operation:" + exception.toString());
			result.addParam(new Param("dbpErrCode", "10213"));
			result.addParam(new Param("dbpErrMsg", "Update T24 failed"));
		}
		return result;
	}
	
	private String getCoreCustomerId(String customerId,DataControllerRequest request) throws Exception {
