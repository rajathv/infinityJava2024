...
public class PDFGeneratorForMortgageSimulationResults {
	...
	public PDFGeneratorForMortgageSimulationResults(JSONArray mortgageSimulatedValues,JSONArray mortgageCurrentValues, Map<String, Object> mortgageFacilityDetails) {
	...
	public String generateFileAndGetBase64() {
		try {
			...
			//adding header
			addHeaderLabel(document, "Mortgage Simulation Results");
	...
	private PdfPTable getTable() {
		...
		for (Map.Entry<String, Object> entry : this.mortgageFacilityDetails.entrySet()) {
			try {
