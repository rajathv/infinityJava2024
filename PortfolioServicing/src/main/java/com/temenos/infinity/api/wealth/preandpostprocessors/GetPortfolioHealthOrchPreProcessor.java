...
public class GetPortfolioHealthOrchPreProcessor implements DataPreProcessor2 {
	private static final Logger LOG = LogManager.getLogger(GetPortfolioHealthOrchPreProcessor.class);
	...
	public boolean execute(HashMap inputMap, DataControllerRequest request, DataControllerResponse response,
		...
		try {
			Set<String> userPermissions = SessionScope.getAllPermissionsFromIdentityScope(request);
			 if (!userPermissions.contains(TemenosConstants.PORTFOLIO_HEALTH_ASSET_ALLOCATION_VIEW)) {
			 ...
			 String portfolioId = null, portfolioservicetype = "";
				...
				LOG.error("Error in GetPortfolioHealthOrchPreProcessor" + e);
				e.getMessage();
				return false;
			}
		}
		private boolean unauthAccess(Result result, String param) {
			...
			result.addParam("httpStatusCode", "0");
			return false;
