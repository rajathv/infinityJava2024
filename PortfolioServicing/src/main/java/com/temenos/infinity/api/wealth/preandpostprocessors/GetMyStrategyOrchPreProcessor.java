...
public class GetMyStrategyOrchPreProcessor implements DataPreProcessor2 {
	private static final Logger LOG = LogManager.getLogger(GetMyStrategyOrchPreProcessor.class);
	...
	public boolean execute(HashMap inputMap, DataControllerRequest request, DataControllerResponse response,
		...
		try {
			Set<String> userPermissions = SessionScope.getAllPermissionsFromIdentityScope(request);
			 if (!userPermissions.contains(TemenosConstants.MY_STRATEGY_VIEW)&&
	            		!userPermissions.contains(TemenosConstants.PORTFOLIO_REVIEW_STRATEGY_VIEW)) {
			 ...
			 String portfolioId = null, portfolioservicetype = "";
				...
				LOG.error("Error in GetMyStrategyOrchPreProcessor" + e);
				e.getMessage();
				return false;
			}
		}
