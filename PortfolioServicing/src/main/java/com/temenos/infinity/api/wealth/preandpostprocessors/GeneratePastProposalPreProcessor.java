...
public class GeneratePastProposalPreProcessor implements DataPreProcessor2 {
	...
	public boolean execute(HashMap inputMap, DataControllerRequest request, DataControllerResponse response,
		...
		try {
			...
			if (!request.getParameter("wealthCore").equalsIgnoreCase("T24,Refinitiv") 
					&& !request.getParameter("wealthCore").equalsIgnoreCase("T24")) {
			...
			LOG.error("Error in ConfirmRecomStratPreProcessor-T24" + e);
			e.getMessage();
		}
		return false;
	}
