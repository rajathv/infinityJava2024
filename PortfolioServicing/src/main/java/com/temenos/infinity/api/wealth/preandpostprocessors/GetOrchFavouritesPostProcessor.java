		...
		if (t24List != null) {
			for (Map.Entry instr : t24List.entrySet()) {
				...
				if (refObj == null) {
					...
					if (refObj.has("lastRate") && !refObj.get("lastRate").equals("")) {
						instrument.put("lastRate", refObj.get("lastRate").toString());
					} else {
						instrument.put("lastRate", t24Obj.getString("marketPrice"));
					}
		...
		if (sortValue.equalsIgnoreCase(TemenosConstants.INSTRUMENTNAME)
			...
			Collections.sort(jsonValues, new Comparator<JSONObject>() {
				...
				public int compare(JSONObject a, JSONObject b) {
					...
					Double dbl1 = null;
					Double dbl2 = null;
					if (a.get(KEY_NAME).equals("")) {
						dbl1 = 0.0;
					} else {
						dbl1 = a.has(KEY_NAME) ? Double.parseDouble((a.get(KEY_NAME)).toString()) : 0.0;
					}
					if (b.get(KEY_NAME).equals("")) {
						dbl2 = 0.0;
					} else {
						dbl2 = b.has(KEY_NAME) ? Double.parseDouble((b.get(KEY_NAME)).toString()) : 0.0;
					}
