...
public class ConfirmRecomStratOrchPreProcessor implements DataPreProcessor2 {
	private static final Logger LOG = LogManager.getLogger(ConfirmRecomStratOrchPreProcessor.class);
	...
	public boolean execute(HashMap inputMap, DataControllerRequest request, DataControllerResponse response,
		...
		try {
			Set<String> userPermissions = SessionScope.getAllPermissionsFromIdentityScope(request);
			 if (!userPermissions.contains(TemenosConstants.RECOMMENDED_STRATEGY_CONFIRMATION)&&
	                    !userPermissions.contains(TemenosConstants.RECOMMENDED_STRATEGY_ACKNOWLEDGEMENT)&&
	                    !userPermissions.contains(TemenosConstants.RECOMMENDED_STRATEGY_ACKNOWLEDGEMENT_CHART)) {
