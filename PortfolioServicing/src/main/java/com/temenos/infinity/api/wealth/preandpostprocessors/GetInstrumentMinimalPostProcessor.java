		...
		if (statusVal.equals("success")) {
			...
			if (bodyDataset != null) {
				...
				String[] colArray = new String[] { TemenosConstants.INSTRUMENTID, TemenosConstants.INSTRUMENTNAME,
						TemenosConstants.STOCKEXCHANGE, TemenosConstants.ISIN, TemenosConstants.RICCODE, "lastPrice",
						"lastPriceDate", "priceCurrency", "instrumentCurrencyId", TemenosConstants.QUANTITY };

				for (int i = 0; i < bodyArray.length(); i++) {
					JSONObject instrObj = bodyArray.getJSONObject(i);
					for (String key : colArray) {
						if (instrObj.has(key)) {
							if (key.equalsIgnoreCase("lastPriceDate")) {
								try {
